...
* **COFFEE_TOTAL**: Total number of calories attributable to coffee consumption
* **GENDER**: Gender
...
Reading in the dietary dataset and selecting the desired variables.
```{r}
diet <- read.xport("DR1IFF_I.XPT")

(diet <- as_tibble(diet) %>%
  select(SEQN, KCAL = DR1IKCAL, USDA = DR1IFDCD))
```

Reading in the food code dataset and selecting the desired variables.
```{r}
...
## Calculating Total Calories from Coffee - `coffee`
...
Finding the total number of calories attributable to consuming coffee or coffee-related products.
```{r}
coffee <- inner_join(join, coffee_code, by = "USDA")

coffee <- coffee %>%
  group_by(SEQN) %>%
  mutate(COFFEE_TOTAL = sum(KCAL)) %>%
...
## Distribution of calories from coffee consumption
```{r, message = FALSE, warning = FALSE}
...
## Distribution of calories from coffee consumption by gender
```{r, message = FALSE, warning = FALSE}
...
## Distribution of calories from coffee consumption by age
```{r, message = FALSE, warning = FALSE}
final %>%
  ggplot() + 
...
## Distribution of calories from coffee consumption by race
```{r, message = FALSE, warning = FALSE}
final %>%
  ggplot() + 
...
## Distribution of calories from coffee consumption by education
```{r, message = FALSE, warning = FALSE}
final %>%
  filter(!(EDUCATION == "Unsure" | is.na(EDUCATION))) %>%
  ggplot() + 
  geom_histogram(mapping = aes(x = COFFEE_TOTAL, fill = EDUCATION), color = "white") +
  labs(title = "Distribution of calories from coffee consumption by education",
       x = "Coffee consumption (calories)",
       y = "Count") +
...
final %>%
  ...
  ggplot() + 
  geom_bar(mapping = aes(x = EDUCATION, y = COFFEE_EDUCATION), stat = "identity") +
  labs(title = "Average calories from coffee consumption by education",
       x = "Education",
       y = "Average calories") +
  theme_minimal() +
  coord_flip() + 
...
final %>%
  ...
  ggplot() + 
  geom_bar(mapping = aes(x = RACE, y = COFFEE_RACE), stat = "identity") +
  labs(title = "Average calories from coffee consumption by race",
       x = "Race",
       y = "Average calories") +
  theme_minimal()
```
